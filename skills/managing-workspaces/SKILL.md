...
- Include common build tools and package managers
   ...
   ```json
   {
     "permissions": {
       "allow": [
         "Write(/absolute/path/to/workspaces/{ticket-id}/**)",
         "Edit(/absolute/path/to/workspaces/{ticket-id}/**)",
         "Read(/absolute/path/to/workspaces/{ticket-id}/**)",
         "Read(/absolute/path/to/source/repos/**)",
         "Bash(git status:*)",
         "Bash(git diff:*)",
         "Bash(git add:*)",
         "Bash(git commit:*)",
   ...
   **Note:** This configuration intentionally excludes `git push` and other remote-destructive operations to ensure user confirmation is required before pushing to remote repositories. Linear MCP permissions are read-only to allow exploring tickets, projects, and documentation without risk of accidental modifications.
